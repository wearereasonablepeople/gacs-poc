...
export class SubmissionsUseCase {
  ...
  async startSubmission(questionnaireId: string) {
    ...
    if (!q || !(q as any).isPublished) throw new BadRequestException('Questionnaire not found or not published');
    return this.submissionRepo.create({ questionnaireId, startedAt: new Date() } as any);
  }
  ...
  async saveAnswer(submissionId: string, questionId: string, selectedOptionId: string) {
    ...
    return this.submissionRepo.upsertAnswer(submissionId, questionId, selectedOptionId);
  }
  ...
  async findByQuestionnaire(questionnaireId: string) {
    const submissions = await this.submissionRepo.findByQuestionnaire(questionnaireId);
  ...
  async findByTenant(tenantId: string) {
    const submissions = await this.submissionRepo.findByTenant(tenantId);
    return submissions.map((s: any) => {
      const totalQuestions = s.questionnaire.sections.reduce(
        (sum: number, sec: any) => sum + sec._count.questions, 0,
      );
  ...
  async getPdfData(id: string) {
    ...
    const answerMap = new Map<string, { id: string; label: string; groupLabel?: string }>(
  ...
  async exportByTenant(tenantId: string) {
    return this.submissionRepo.exportByTenant(tenantId);
  }
}
