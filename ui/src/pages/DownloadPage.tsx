...
export function DownloadPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const { data } = await api.get<PdfApiResponse>(`/submissions/${submissionId}/pdf-data`);
        ...
        setPageState('error');
        setErrorMessage('Kon de inzendingsgegevens niet ophalen. Probeer het later opnieuw.');
      }
    };
  ...
  useEffect(() => {
    ...
    if (tenant.primaryColor) {
      ...
      const fg = isLightColor(tenant.primaryColor) ? '222 47% 11%' : '210 40% 98%';
      document.documentElement.style.setProperty('--primary-foreground', fg);
    }
    if (tenant.secondaryColor) {
      document.documentElement.style.setProperty('--accent', hexToHSL(tenant.secondaryColor));
    }
  ...
  const textColor = tenant?.headerTextColor
    || (tenant?.primaryColor ? (isLightColor(tenant.primaryColor) ? '#1e293b' : '#ffffff') : undefined);
  ...
  return (
    <div className="min-h-screen bg-muted/50" style={pageBackground}>
      {/* Header */}
      <header className={tenant?.primaryColor ? 'shadow-sm' : 'border-b bg-background'} style={headerStyle}>
        <div className="mx-auto max-w-3xl flex items-center gap-3 p-4">
          {tenant?.logoUrl ? (
            <img
              src={tenant.logoUrl}
              ...
              style={tenant.primaryColor ? { backgroundColor: 'rgba(255,255,255,0.2)', padding: '2px' } : undefined}
            />
          ) : tenant?.primaryColor ? (
            <div
      ...
      <div className="mx-auto max-w-2xl p-4 pt-8">
        <Card>
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Uw antwoorden voor <strong>{questionnaireTitle}</strong> zijn succesvol ingediend en geverifieerd.
              U kunt hieronder een PDF downloaden met al uw antwoorden.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {/* Summary info */}
            <div className="rounded-lg border bg-muted/50 p-4 space-y-2">
              ...
              {pdfData?.respondentEmail && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-muted-foreground">Geverifieerd e-mailadres:</span>
              ...
              {pdfData?.submittedAt && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-medium">{new Date(pdfData.submittedAt).toLocaleString('nl-NL')}</span>
                </div>
              )}
              <div className="flex items-center gap-2 text-sm">
                <FileText className="h-4 w-4 text-muted-foreground" />
                <span className="text-muted-foreground">Aantal secties:</span>
                <span className="font-medium">{pdfData?.sections.length ?? 0}</span>
              </div>
            </div>
            ...
            <p className="text-xs text-center text-muted-foreground">
              U kunt deze pagina bookmarken om later terug te komen en de PDF opnieuw te downloaden.
            </p>
            <p className="text-xs text-center text-muted-foreground">
              Wilt u uw gegevens laten verwijderen?{' '}
              <a href="/gdpr/erasure" className="underline text-primary hover:text-primary/80">
